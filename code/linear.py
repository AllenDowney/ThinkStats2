...
def PlotConfidenceIntervals(xs, inters, slopes, res=None, percent=90, **options):
...
def PlotSamplingDistributions(live):
    ...
    thinkplot.Save(
        root="linear3", xlabel="age (years)", ylabel="birth weight (lbs)", legend=False
    )
...
def PlotResiduals(live):
    ...
    thinkplot.Save(
        root="linear2", xlabel="age (years)", ylabel="residual (lbs)", xlim=[10, 45]
    )
