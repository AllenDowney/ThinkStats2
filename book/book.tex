...
is still useful.  The correlation of height and weight is 0.51, which
is a strong correlation compared to similar human-related variables.
% TODO: forward reference to discussion of strength
...
we see in a sample are likely to hold for the entire population.
For example, in the NSFG sample we see a difference in the mean pregnancy
...
\item The last step is to interpret the result.  If the p-value is
  ...
  we infer that the effect is more likely to be real; that is, to
  reflect an effect in the general population.  \index{statistically
    significant} \index{significant}

\end{itemize}

The underlying logic of this process is similar to a proof by
...
{\tt HypothesisTest} is an abstract parent class.  It provides
implementation of \verb"__init__" and {\tt PValue}.  Child
classes inherit these methods and provide implementation for
{\tt TestStatistic}, {\tt RunModel}, and optionally {\tt MakeModel}.

\verb"__init__" takes as a parameter the data in whatever form
is appropriate.  It passes the data to {\tt TestStatistic}, which
computes the size of the effect in the actual sample.  It also
...
the number of times to simulate the null hypothesis.  It calls
{\tt RunModel}, which returns the result of the simulation, which
should be ``fake'' data in the same format as the actual data.

The first line of {\tt PValue} computes the test statistic for each
simulated dataset and stores the results: \verb"test_stats" is the
list of simulated test statistics; \verb"test_cdf" is their CDF.  The
p-value is just the fraction of elements in \verb"test_stats" that
exceed or equal the observed test statistic, {\tt self.actual}.

As a simple example\footnote{}, suppose we toss a coin 250 times and
see 140 heads and 110 tails.  Based on this result, we might suspect
that the coin is biased; that is, more likely to land heads.  To test
this hypothesis, we will compute the probability of seeing such a big
difference if the coin is actually fair.

{\tt CoinTest} is child class of {\tt HypothesisTest} that answers
this question:
...
For this example, I represent the data as a pair of
integers: the number of heads and tails.  The test statistic is
the absolute difference between them, so {\tt self.actual}
is 30.

{\tt RunModel} simulates coin tosses assuming the null hypothesis
that the coin is actually fair.  It
generates a sample of 250 tosses,
uses a Hist to count the number of heads and tails, and returns
a pair of integers.
...
The result is about 0.07, which means that if the coin is actually
fair, we expect to see a difference as big as 30 about 7\% of the
time.

How should we interpret this result?  In practice, people often
consider 5\% to be the threshold of statistical significance.  If the
...
p-value we get depends on the choice of the test statistics and
the model of the null hypothesis.  So p-values should not be considered
precise measurements.

Instead I recommend interpreting p-values according to their order
of magnitude: if the p-value is less than 1\%, the effect is unlikely
to be due to chance; if it is greater than 10\% it can plausibly
be explained by chance.  P-values between 1\% and 10\% should be
considered borderline.  So in this example I would conclude that
the data do not provide strong evidence that the coin is biased
or not.
...
hypothesis is by {\bf permutation}; that is, we can take birth
weights for first babies and others and shuffle them, treating
the two groups as one big group.
\index{null hypothesis}

Here's the code that tests a difference in means by permutation:
...
The data are represented with a pair of sequences, one for each
...
pool of values and splitting into two groups with sizes {\tt n}
...
Then we extract pregnancy lengths as NumPy arrays, pass them as
...
distribution of the test statistic, with a gray line indicating
...
Figure~\ref{hypothesis1} shows the result.  The gray line indicates
the observed difference, 0.078 weeks.  The CDF intersects the gray
line at 0.83, which is the complement of the p-value, 0.17. 

If we run the same analysis with birth weight, the computed p-value
is 0, which means that the simulation never yielded an effect
as big as the observed difference, 0.12 lbs.  After 1000 attempts,
the biggest simulated difference is 0.10 lbs.  So we would
report that the p-value is less than $1/1000$, or 0.001, and
...
trying to address, and sometimes depends on
the distribution of the data.

For example, if the relevant question is whether pregnancy lengths
are different, one way or the other, for first babies, then it makes
sense to test the absolute difference in means, as we did in the
previous section.

If we had some previous reason to think that first babies are likely
...
previous test, using both sides, is called {\tt two-sided}.
...
The one-sided hypothesis, that first babies are born late, is
more specific that the two-sided hypothesis, so the p-value is smaller.
But in this example the result, even for the stronger hypothesis,
is not statistically significant.

Even if mean pregnancy length is the same for first babies and others,
maybe the standard deviation is different.  In Section~\ref{}, we
saw some evidence that first babies are more likely to be early or
late, and less likely to be on time.  So we might hypothesize that
the standard deviation is higher.  Here's how we can test that:
...
This is a one-sided test because the hypothesis under test is
that the standard deviation for first babies is higher, not just
different.  The p-value is 0.09, again not statistically
significant. 


\section{Other models}

In the previous section we simulated the null hypothesis by
...
An alternative is to compute the distribution of the observed
values and then draw a random sample from that distribution.
This process is called {\bf resampling}.  Here's how you could
test a difference in means using resampling:
...
This version inherits {\tt TestStatistic} and {\tt MakeModel} 
from {\tt DiffMeansPermute}.  The only difference is in {\tt RunModel},
which draws random samples from the observed values with replacement.

Because the same value might be selected more than once, resampling is
less constrained than permutation, so we expect more variability in
the test statistic and a higher p-value.  But the difference is small:
the p-value for the difference in pregnancy lengths is 0.17 by
resampling, compared to 0.16 by permutation.
...
We can use the {\tt HypothesisTest} framework to test whether
an observed correlation is statistically significant.

For example, in the NSFG data set, the correlation between
birth weight and mother's age is xxx.  Apparently older mothers
tend to have heavier babies.  But could this effect be due to
chance?

To answer that question, we have to choose a test statistic.
If we had reason to believe that the correlation was positive,
we would do a one-sided test using correlation as the test
statistic.  But if we had not background knowledge, we would
probably do a two-sided test using the absolute value of
correlation.
...
simulate an alternate universe where the distributions of age and
...
{\tt data} is a tuple of sequences.  The test statistic is the
absolute value of Pearson's correlation.  {\tt RunModel}
uses the NumPy function {\tt permutation}, rather than {\tt shuffle},
because it returns a new array without modifying the data.
...
In this case we use {\tt dropna} to drop any rows that are
missing either of the variables we need.

The computed p-value is 0;
after 1000 iterations the largest simulated correlation is 0.04,
which is less than the actual correlation, 0.07.  So although
the observed correlation is small, it is statistically significant.

This example is a reminder that ``statistically
significant'' does not always mean that an effect is important,
or significant in practice.  With a large enough data set, almost
any correlation, or difference between groups, will be statistically
significant.  But it's the size of the effect, not the p-value,
that indicates whether the effect is important.
...
frequencies, and then add up the absolute differences.  In this
example, the differences are -2, -1, 9, -5, -2, and 1, so the total
absolute difference is 20.  How often would we expect to see
a difference as big as that by chance?
...
The data are represented as a list of frequencies, so the observed
values are {\tt [8, 9, 19, 5, 8, 11]} and the expected frequencies
are all 10.  The test statistic is the sum of the absolute differences.

The null hypothesis is that the die is fair, so we simulate
that by drawing random samples from {\tt values}.
{\tt RunModel} uses a Hist to compute and return the list of frequencies
...
Squaring the deviations (rather than taking absolute value) gives
...
Using a different test statistic yields different results.  In this
example, the p-value using the chi-squared statistic is 0.04,
substantially smaller than what we got with total deviations, 0.12.
...
Naturally this procedure raises two question:
...
\item If there is no real effect, the null hypothesis is true,
  so we can compute the
  distribution of the test statistic by simulating the null
  hypothesis.  Suppose we store that distribution in a Cdf called
  \verb"test_cdf".

\item Each time we run an experiment, we get a test statistic, {\tt
  stat}, which is drawn from \verb"test_cdf".  Then we compute a
  p-value.  The p-value is the probability that a random value from
  \verb"test_cdf" exceeds {\tt t}, which is \verb"1 - test_cdf[t]".

\item The p-value is less than 5\% if \verb"test_cdf[t]" is greater
  than 95\%; that is, if {\tt t} exceeds the 95th percentile.

\item And how often does a value chosen from \verb"test_cdf" exceed
...
the actual effect size, and normally we don't know that.  But the
false negative rate can be used to interpret a negative test result.

\section{Power}
\label{power}

For example, in Section~\ref{???} we found that the apparent difference
in mean pregnancy length is not statistically significant.  Does that
mean there is no difference at all?  Not necessarily.  The actual
effect might be smaller, or we might have been unlucky.  To interpret
this result, it is helpful to know the false negative rate,
conditioned on a hypothetical effect size.

One way to do that is to use the observed samples as a model of the
population and run simulated experiments by resampling.
Here's the code:
...
The result is about 68\%, which means that if the estimated difference
is correct and the actual difference in mean pregnancy length is 0.78
weeks, we expect an experiment with this sample size to yield a
negative test 68\% of the time.

This result is often presented the other way around: if the actual
difference is 0.78 weeks, we should expect a positive test only 32\%
...
In this example, the test had only a 32\% chance of yielding a
...
we could say that this test was ``underpowered.''

Thus the negative result does not imply that there is no difference
between the groups; the strongest conclusion we can draw is that if
there is actually a difference, it is probably too small to detect
...
Hypothesis testing is one of the most problematic areas of statistics.
It is poorly understood, often done wrong, and more often interpreted
incorrectly.  It is the topic of endless debates, some based on
mistakes, others based on philosophical disagreements.  I won't try to
catalog everything that can go wrong, but here are some common
problems:

\begin{itemize}

\item Multiple tests: If you run one hypothesis test, the chance of a
  false positive is about 1 in 20.  If you run 20 tests, you should
  expect at least one false positive, most of the time.  If you ignore
  the negative results and publish the positive ones, most of what you
  publish will be wrong.

\item Exploring and testing with the same data: Ideally you should
  decide what to test, and how, before looking at the data.  If you
  explore a large dataset, find a surprising effect, and then test
  whether it is significant, you have a good chance of generating a
  false negative.

\item Treating the p-value as the result: For practical purposes the
  size of the effect is more important than the p-value.  For example,
  if you are comparing two groups, there is almost always some
  difference between them.  If the sample size is large enough, the
  difference will be statistically significant, even if it has no
  practical effect.

\end{itemize}

Most of these problems are easy to address once you are aware of them.
For example, if you run multiple tests, you can adjust the p-value
threshold to compensate.  For details, see
\url{https://en.wikipedia.org/wiki/Holm-Bonferroni_method}.

To avoid exploring and testing with the same data, you can partition
data into two parts, then use one for exploration and the other for
testing.  Although this approach is not widely used, a related
practice is: replication of previous results.  Informally, many
scientists consider the first paper to report a new result to be
exploratory.  Subsequent papers that replicate the result with new
data are considered confirmatory.

As it happens, we have an opportunity to confirm the results in this
chapter.  The first edition of this book is based on Cycle 6 of the
NSFG, which was released in 2002.  In October 2011, CDC released
additional data based on interviews conducted from 2006--2010.
...
Make a scatter plot of mother's age and baby's weight for each live
