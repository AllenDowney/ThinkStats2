...
{\tt groupby} is a DataFrame method that returns a DataFrameGroupBy
object, which is a map from each group name to a DataFrame that
contains the elements in the group.  Since the values of {\tt quality}
...
be available.  We will consider ways to deal with missing data like
this.

Visually, it looks like the price of high quality cannabis is
declining during this period, and the price of medium quality
might be increasing.  The price of low quality might also be
increasing, and it seems to be more volatile.  Keep in mind
that the quality data is reported by participants in the study,
so trends over time might reflect changes in how participants
apply these categories.


\section{Linear regression}

Although there are methods specific to time series
analysis, for many problems a simple way to get started is by
applying general-purpose tools like linear regression.  The following
function takes a DataFrame of daily prices and computes a
least squares fit, returning the model and results objects
from StatsModels:
...
it increase by 57 cents per year.  These estimated slopes are
...
for the data, but it's not clear that the long-term trend
is linear.  Visually, it seem like there is a change at around 2
years, with decreases in the rate of decline and variability.
...
\item Sampling error: The prediction is based on the parameters of
the model, but the estimated parameters depend on random variation
in the sample.  If we ran the experiment again, we would expect
the estimates to vary.

\item Random variation:  Even if the estimates parameters are
...
two states, and legal for medical purpose in 20 more.  If more states
legalize it, the price is likely to go down.  On the other hand, if
the federal government cracks down, the price might go up.
You can think of these unpredictable changes as a fourth kind
of error, or as a variety of model error.

Either way, modeling errors and unexpected future events are hard to
quantify.  They are what Donald Rumsfeld would call ``unknown
unknowns'' (see
\url{https://en.wikipedia.org/wiki/There_are_known_knowns}).  Sampling
error and random variation are easier to deal with, so let's do that.

To quantify sampling error, I'll use resampling, as we did in
Section~\ref{regest}.  In that example, we resampled the data;
that is, we selected random rows from the DataFrame of observations.
...
\caption{Predictions based on the linear fit.}
\label{timeseries4}
\end{figure}
