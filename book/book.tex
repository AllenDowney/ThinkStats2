...
In the previous examples, some of the explanatory variables are
numerical, some categorical, and some boolean (which is really
categorical).  But the dependent variable was numerical.
...
process is called {\bf logistic regression}.  If the dependent
variable is a positive, integer count, it's called {\bf Poisson
regression}.  These forms of regression are known as
{\bf generalized linear models}.

Logistic regression is used when the dependent variable is boolean.
For example, thinking again about the office pool scenario, suppose
...
factors that affect the probability of having a girl.

If you encode the outcomes numerically, for example 0 for a girl
and 1 for a boy, you could apply ordinary least squares to this
problem.  The linear model might be something like this:
%
\[ y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \eps \]
%
...
$x_1$ and $x_2$ has a 50\% chance of having a girl.  But it is also
possible for this model to predict $y=1.1$ or $y=-0.1$, and those
are not valid probabilities.

Logistic regression solves this problem by expressing predictions in
terms of {\bf odds} rather than probabilities.  You might have heard
expressions like ``the odds are three to one,'' but you might not know
what that means.  The ``odds in favor'' of an event refer to the
ratio of the probability it will occur to the probability that it will
not.
...
represent them as a ratio of integers.  So ``three to one'' is
written $3:1$.

When probabilities are low, it is more common to report the
``odds against'' rather than the odds in favor.  For
example, if my horse has a 10\% chance of winning,
the odds against are $9:1$.

Odds and probabilities are different representations of the
...
If you represent odds as a ratio {\tt x:y}, you
can convert to probability like this:

\begin{verbatim}
def Probability2(x, y):
    return x / (x + y)
\end{verbatim}
...
values for $x_1$ and $x_2$.  We could compute the predicted value of
$\log o$, and then convert to a probability:

\begin{verbatim}
def Probability3(log_o):
    o = math.exp(log_o)
    return o / (o+1)
\end{verbatim}
...
which is the parameter values that maximize the likelihood of the
...
And we start with the initial guesses $\beta_0=-1.5$, $\beta_1=1.3$,
and $\beta_2=1.8$:
...
Then we can convert from log-odds to odds:

\begin{verbatim}
>>> o = np.exp(log_o)
[ 0.223,  1.349,  0.818,  4.953]
\end{verbatim}

And from odds to probabilities:

\begin{verbatim}
>>> p = o / (o+1)
[ 0.182,  0.574,  0.450,  0.832]
\end{verbatim}

Notice that when log-odds are greater than 0, the correspond to
odds greater than 1 and probabilities greater than 0.5.

If we think the probability of a boy is 0.8 and the outcome is a boy,
the likelihood of the outcome is 0.8; if the outcome is a girl, the
likelihood is 0.2.  More generally, the likelihood of an outcome is 
{\tt p} when {\tt y==1} and {\tt 1-p} when {\tt y==0}.  We can
compute that like this:

\begin{verbatim}
>>> likes = y * p + (1-y) * (1-p)
[ 0.817,  0.574,  0.549,  0.832]
\end{verbatim}
...
For these values of {\tt beta}, the likelihood of the data is 0.215.
To find the values of {\tt beta} that maximize likelihood, most
statistics packages use an iterative solver like Newton's method
(see \url{https://en.wikipedia.org/wiki/Logistic_regression#Model_fitting}).


\section{Implementation}

{\tt statsmodels} provides an implementation of logistic regression
called {\tt logit} (it is named for the logit function, which converts
from probability to log-odds).  We can use it to look for variables
that help predict the sex of a baby.

Again, I loaded the NSFG data and selected pregnancies longer than
30 weeks:
...
{\tt logit} requires the dependent variable to be binary (rather
than boolean), so I'll create a new column:

\begin{verbatim}
    df['boy'] = (df.babysex==1).astype(int)
\end{verbatim}

{\tt astype} is a Series method that converts to other types.  Now
we can run a logistic regression to see if the mother's age affects
the sex of the baby:
...
Section~\ref{???} presents a hypothetical office pool where friends
of a pregnant woman try to guess the baby's birth weight.  Suppose,
instead, that the goal is to predict the date of birth.  Assuming that
bets are placed during the 30th week of pregnancy, what variables
could you use to make the best prediction?  You should limit yourself
to variables that are known before the birth, and likely to be available
...
ratio of boys to girls depends on ``maternal condition''; that is,
factors like the mother's age, size, health, and social status.
\url{https://en.wikipedia.org/wiki/Trivers-Willard_hypothesis} Some
studies have shown this effect among humans, but results have been
mixed.

Use the NSFG data to test this hypothesis.  Can you evidence that
