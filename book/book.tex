...
From {\tt transactions} we select {\tt date} and {\tt ppg}, then
group by {\tt date}.  The result, {\tt grouped}, is a map from
each date to a DataFrame that contains the prices reported
on that date.  {\tt aggregate} is a DataFrameGroupBy that iterates
through the groups and applies a function to each column of the
group; in this case there is only one column, {\tt ppg}.  So the
result of {\tt aggregate} is a DataFrame with one row for each
date and one column, {\tt ppg}.

At this point {\tt daily} contains rows for each date with at
least one reported transaction, but since there are days with no
transactions, there are gaps.  To make the time series easier to work
