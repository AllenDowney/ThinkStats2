...
that by {\bf reindexing} the DataFrame:
...
If the price is above the long-term trend on Monday,
you might expect it to be above the trend for a few more days; and
if it's below the trend, you might expect it to stay below.  A pattern
...
by an interval called a {\bf lag}, and the compute the correlation
...
With lag=1, the serial correlations for the price data are
...
Based on these results, it looks like there are no seasonal patterns
in this data, at least not with these lags.
...
analysis, including {\tt acf}, which computes the
...
confirm that {\tt acf} and {\tt SerialCorrelation} yield approximately
the same results:
...
correlations are highest when the lag is 7 or a multiple of 7.  For
...
make predictions.  In particular, price predictions
can help people choose the best time to buy or sell.
...
Each time through the loop, we generate 
...
\begin{exercise}
Test serial correlation for significance.
\end{exercise}
