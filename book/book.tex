...
roughly {\tt inter + slope * x[i]}.  \index{residual}
...
\item If the residuals are uncorrelated and
  normally distributed with $\mu = 0$ and constant (but unknown)
  $\sigma$, then the least squares fit is also the maximum likelihood
  estimator of {\tt inter} and {\tt slope}.  See
...
enough.  {\tt thinkstats2} provides simple functions that demonstrate
...
{\tt xs} and {\tt ys} and return the estimated parameters {\tt inter}
and {\tt slope}.
...
We can use these functions to compute and a least squares fit for
...
\begin{figure}
% regression.py
\centerline{\includegraphics[height=2.5in]{figs/regression1.pdf}}
\caption{Scatter plot of birth weight and mother's age with
a linear fit.}
\label{regression1}
\end{figure}

Figure~\ref{regression1} shows a scatter plot of birth weight and age
...
Another useful test is to plot the vertical deviations from the
fitted line, called {\bf residuals}.  {\tt thinkstats2} provides
a function that computes residuals:
...
Figure~\ref{regression2} shows the 25th, 50th and 75th percentiles of
...
is a simple model that is probably good enough for most purposes.

\begin{figure}
% regression.py
\centerline{\includegraphics[height=2.5in]{figs/regression2.pdf}}
...
5th and 95th percentiles (provided that there are 101 rows).

{\tt FillBetween} draws a polygon that fills the space between two
lines.  Figure~\ref{regression3} shows the result.  The vertical
width of the region represents the effect of sampling error; the
effect is smaller for values near the mean and larger for extreme
values.

\begin{figure}
% regression.py
\centerline{\includegraphics[height=2.5in]{figs/regression3.pdf}}
\caption{Confidence intervals of weight for each age.}
\label{regression3}
\end{figure}
...
If you guess birth weight without knowing the mother's age, 
the RMSE of your guess is {\tt Std(ys)}, which is 1.41 lbs.
So in this example mother's age does not improve the predictions
substantially.
...
(a standardized test used for college admissions in the U.S.) they
...
a better indicator of reduction in MSE, and reduction in
RMSE is a better indicator of predictive power.
...
mother's age and birthweight.  In fact, because $R^2 = \rho_2$, a
...
we can model the birth weights as random variation around their mean.
...
test statistic is the slope estimated by linear least squares.
The model of the null hypothesis is represented by the mean weight
of all babies and the sequence of deviations from the mean.  To
...
Estimating the p-value by simulating the null hypothesis is 
rigorous, but there is a simpler alternative.  Remember that we
already computed the sampling distribution of the slope, in
Section~\ref{regest}.  To do that, we assumed that the observed
slope was correct and simulated experiments by resampling.

Figure~\ref{regression4} shows the sampling distribution of the
...
\begin{figure}
% regression.py
\centerline{\includegraphics[height=2.5in]{figs/regression4.pdf}}
...
hypothesis exceeds the observed slope, 0.017 lbs/year, or
...
The first is more strictly correct, but the second is easier, because
we normally want to compute the sampling distributions of the
parameters anyway.  And it is a good approximation unless the sample
size is small {\em and} the distribution of residuals is skewed.  Even
then, it is good enough, because we only care about the order of
magnitude, usually.
...
\begin{verbatim}
    inters, slopes = SamplingDistributions(live, iters=1000)
    slope_cdf = thinkstats2.Cdf(slopes)
    pvalue = slope_cdf[0]
\end{verbatim}

Again, we find $p < 0.001$.  If the estimated slope were negative,
we would compute the probability that the slope in the sampling
distribution exceeds 0.

\section{Weighted resampling}

So far we have been treating the NSFG data as if it were a
representative sample, but as I mentioned in Section~\ref{nsfg},
it is not.  The NSFG deliberately oversamples several groups
in order to improve the chance of getting statistically significant
results; that is, in order to improve the power of tests involving
these groups.

This survey design is useful for many purposes, but it means that
we cannot use the survey sample to estimate values for the general
...
the respondent represents.  This value is called a ``weight'' in
the sense of ``weight of evidence'' -- not to be confused with birth
weight!

As an example, if you survey 100,000 people in a country of 300
million, each respondent represents 300,000 people.  If you oversample
one group by a factor of 2, each person in the oversampled
group would have a lower weight, about 150,000.

To correct for oversampling, we can use resampling; that is, we
can draw samples from the survey data using probabilities proportional
to {\tt totalwgt}.  For any quantity we want to estimate, we can
...
rows.  Since we sampled with replacement, the same row might appear
more than once.

Now we can compare the effect of resampling with and without
weights.  Without weights, we can generate the sampling distribution
like this:
...
larger than the standard error of the estimate, 0.014 pounds, so
we conclude that the difference is not due to chance.
...
A solution to this exercise is in \verb"chap10ex_soln.py"
