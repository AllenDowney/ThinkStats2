...
\item {\tt MakePmf}, which evaluates the density at a discrete set of
  values and returns a Pmf that approximates the Pdf.
...
The parameters of {\tt MakePdf} are a lower bound, an upper bound, and
the number of points to evaluate, {\tt n}.  This example evaluates
100 points, equally
spaced from three standard deviations below the mean to three standard
deviations above.

{\tt thinkplot.Pdf} plots the resulting Pmf as a smooth function,
...
\begin{verbatim}
    ...
    thinkplot.Pmf(pmf, label='sample KDE')
\end{verbatim}
...
\item[Visualization]: During the exploration phase of a project,
CDFs are usually the best visualization of a distribution.  After you
look at a CDF, you can decide whether an estimated PDF is an appropriate
model of the distribution.  If so, it can be a better choice for
presenting the distribution to an audience that might be unfamiliar
with CDFs.
...
the population distribution is continuous and smooth, you can use
KDE to estimate the density for values that don't appear in the sample.

\item[Simulation]: If your sample is an input to a simulation,
...
rather than replicating the observed sample.  
...
set of values.  To get from a PMF to a CDF, we computed a cumulative
...
by {\tt thinkstats2}: Hist, Pmf, Cdf, and Pdf.  Hist and Pmf inherit
from a parent class called \verb"_DictWrapper".  The underscore
indicates that this class should not be used by code in other modules.
The name indicates what it is: a dictionary wrapper.  The primary
attribute is {\tt d}, the dictionary that maps from values to their
frequencies.
...
% TODO: replace sigma with std in the code
