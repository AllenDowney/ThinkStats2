...
\item If the residuals are uncorrelated and
  ...
  estimator of $\alpha$ and $\beta$.  See
...
cost(\eps_i), and minimize total cost.
\index{cost function}
...
We can use these functions to compute and a least squares
fit for birth weight as a function of mother's age.
...
doesn't make sense in this context, and the slope is too small to
grasp easily.

Instead of presenting the intercept with $x=0$, it
...
To simulate the experiments, I'll assume temporarily that the estimated
parameters are right, and that the residuals represent deviations from
the fitted line due to unknown factors and random noise.  So we can
generate simulated data by permuting the residuals and adding them
to the fitted line.

\begin{verbatim}
def SamplingDistributions(fxs, fys, res, iters=100):
    t = []
    for _ in range(iters):
        random.shuffle(res)
...
{\tt fxs} and {\tt fys} are the fitted values, and {\tt res}
is the sequence of residuals.  {\tt iters} is the number of
experiments to simulate.

Each time through the loop, {\tt SamplingDistributions} shuffles
the residuals, generates simulated data, and used {\tt LeastSquares}
to estimate {\tt inter} and {\tt slope}.  It returns sequences
...
0.07 and confidence interval (6.70, 6.95).  The estimated slope, in more
compact form, is
0.0175, SE 0.0028, CI (0.0126, 0.0226).
...
%
But if I told you their height, you would guess
{\tt inter + slope * x}; in that case your MSE would be $Var(\eps)$.
%
\[ MSE = 
...
% TODO: example here of expressing these things in terms of predictive
% power
