...
So you can also think of a DataFrame as a dictionary that maps from
...
{\tt int64} is one of the data types provided by NumPy.  If you run
...
results of the slice is another Series.
...
This notation doesn't work if the name of the variable has a space in
it, which is one reason the name of the variable should not have a
space in it.
...
{\tt nsfg.py} provides {\tt CleanPregFrame}, a function that cleans
...
The first line transforms {\tt agepreg}, dividing each element by 100.
In the data file, the respondent's age is encoded as an integer
number of centiyears.  Dividing by 100 converts to years.
...
if they are not handled properly, the can easily introduce bogus
values.  The replace method replaces these values with the floating-point
value {\tt NaN}, which represents something that is ``not a number.''
{\tt nan} is available from NumPy, which I imported as {\tt np}.
