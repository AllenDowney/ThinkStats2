...
corresponding value, {\tt x}; that is, the inverse CDF of {\tt p}.
...
simple.  The CDF of the exponential distribution is an analytic
function:
%
\[ \CDF(x) = 1 - e^{-\lambda x} \]
%
...
Figure~\ref{analytic_interarrival_cdf} shows the plotted CDF.
It seems to have the general shape of
an exponential distribution, but how can we tell?
...
%TODO: place these figures side by side

Figure~\ref{analytic_interarrival_ccdf} shows the result.  It is not
...
1.83$ babies per hour.  The mean of an exponential distribution is
$1/\lambda$, so the mean time between babies is 0.55 hours.


\section{The normal distribution}
\label{normal}

The normal distribution, also called Gaussian, is commonly
...
%
This integral does not have a closed form solution, but there are
algorithms that evaluate it efficiently.  One of them is provided
by SciPy: {\tt scipy.stats.norm} is an object that represents
a normal distribution; it provides a method, {\tt cdf}, that evaluates
...
The median of the standard normal distribution is 0, so
$\CDF(0)$ is 0.5.

{\tt norm.cdf} takes optional parameters: {\tt loc}, which
specifies the mean, and {\tt scale}, which specifies the
standard deviation.
{\tt thinkstats2} provides {\tt GaussianCdf}, which uses
{\tt norm.cdf}:
...
Figure~\ref{analytic_gaussian_cdf} shows the CDF of normal distributions
with a range of parameters.  The sigmoid shape of
these curves is a recognizable characteristic of a normal distribution.
...
by choosing $p$ from a uniform distribution from 0 to 1, then choosing
%
\[ x = ICDF(p)\]
%
...
compare it to Figure~\ref{analytic_interarrival_ccdf}.
\index{pyplot}
