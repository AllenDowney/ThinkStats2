...
and returns the resulting density\footnote{The word ``Gaussian'' appears
in the name because it uses a Gaussian filter to smooth
the KDE.}.  
\index{density}
...
to merge them into a single DataFrame.  The option \verb"ignore_index"
...
My solutions to these exercises are in \verb"chap13ex_soln.py".
...
analytic solutions, which can be much faster.
...
preserve.  Having weighed 9 gorillas, you find $\xbar=90$ kg and
...
To answer that question, we are going to compute the sampling
...
The result is an
approximation of the sampling distribution.  The standard deviation
of the sampling distribution is the standard error, which turned
out to be about 2.5 kg.  The interval between the 5th and 95th percentile
of the sampling distribution is a 90\% confidence interval.

Now we'll do the same calculation analytically.  To do that, we
can take advantage of the fact that the weights of adult female
gorillas are roughly normally distributed.  Normal distributions
have two properties that make them amenable for analysis:
they are
closed under linear transformation and convolution.  To explain what
that means, I will need some notation.
\index{analysis}
...
And the standard deviation of the sampling distribution, which is
the standard error of the estimate, is $\sigma / \sqrt{n}$.  In
the example, $\sigma$ is 7.5 kg and $n$ is 9, so the standard error is
2.5 kg.  That result is consistent with what we computed by simulation,
...
function a little easier to use:
...
we can compute the 5th and 95th percentiles like this:
...
Lot of things in the real world are normally distributed, at least
approximately.  So the methods in this section have many applications.
That would be good enough, but with the Central Limit Theorem, things
are even better!
...
This is called the {\bf Central Limit Theorem} (CLT).  It is one of the
...
correlated, the CLT doesn't apply.
\index{independent}
...
animals and other life forms are affected by genetic
...
To test the Central Limit Theorem, I will compute distributions
of sums.  In general, there are three ways to compute the distribution
of a sum:

\begin{itemize}

\item We can draw random samples from the distributions, add them
up, and accumulate the distribution of the sums, or

\item Given two Pmfs, we can enumerate all pairs of values and
compute their sums and probabilities,

\item We can {\bf convolve} the two distributions.

\end{itemize}

I present these methods in the following sections.


\section{Random samples}

Suppose we are given a Cdf of a distribution that is not necessarily
normal.  We would like to know the distribution of the sum of a sample
with size {\tt n}.  The simplest way to compute it is to draw random
samples and add them up:

\begin{verbatim}
\end{verbatim}

Distribution of sum of pregnancy lengths
...
Suppose we have two Pmfs that represent the distributions of
two quantities.  To compute the distribution of the sum,

Differences of sums of pregnancy lengths
...
random variables, $X$ and $Y$, represented
...
\section{Difference in means test}
\index{resampling}
...
\section{Discussion}
  ...
  These methods
have several advantages over analysis: 
...
\item Use computational methods during exploration.  When you get to a
  satisfactory answer, if the run time is acceptable, stop.

\item If run time is not acceptable, look for opportunities to
  optimize.  Using analytic methods is one of several methods of
  optimization.

\item If replacing a computational module with an analytic module is
  appropriate, use the computational model to establish regression
  tests, which provide mutual validation between the computational and
  analytic results.
...
Chapter~\ref{modeling} to see if the sum can be modeled by an
