...
\item {\bf data cleaning}: processes that include validating data, identifying
...
read the documentation at {\tt thinkstats2.com/thinkstats2.html}.
...
Now we are ready to get back to the question from Chapter~\ref{intro}:
``Are first babies more likely to be late?.''  The code in
this chapter is from {\tt first.py}.  For information
about downloading and working with this code, see Section~\ref{code}.

I start by selecting the rows from the pregnancy file where the outcome
is a live birth, and then partition them into first births and others:
...
Before going any farther, it is always a good idea to validate
the data by checking for {\bf outliers}.  Outliers are extreme
values that might be caused my errors in measurement and recording,
or might be accurate measurements of rate events.

In the list of pregnancy lengths for live births, the 10 lowest values
are \{0, 4, 9, 13, 17, 17, 18, 19, 20, 21\}.  Values below 10 weeks
...
The best way to handle outliers depends on {\bf domain knowledge};
...
their histograms on the same axis.  Here's the code from {\tt first.py}:
...
\verb"first_hist" and \verb"other_hist" are histograms of
...
appearance of the histograms.  I chose a width that leaves some
...
An {\bf effect size} is a summary statistic intended describe, well,
the size of an effect.  For example, to describe the difference
between two groups, one obvious choice is the difference in the
means.
\index{effect size}

For example, the mean pregnancy length for first babies is 38.6;
for other babies it is 38.5.  The difference is 0.078 weeks, which
works out to 13 hours.

If we assume that this estimate is accurate, the difference in
pregnancy length would have no practical consequences.  Without
observing a large number of pregnancies, it is unlikely that anyone
would notice a difference this small.

As a fraction of the typical pregnancy length, this difference is
about 0.2\%.
...
It is appropriate to design statistical report and visualizations and
other statistics clearly and effectively.  But you should also do your
...
\begin{exercise}
Based on the results in this chapter, suppose you were
...
{\tt chap02ex.py}.  You can find a solution in
\verb"chap02ex_soln.py".
...
\begin{exercise}
Make a histogram for some other variables ???
\end{exercise}
...
Figure~\ref{nsfg_pmf} shows PMFs of pregnancy length for first babies
...
The code that generates this figure is in {\tt first.py}.
...
Figure~\ref{nsfg_diffs} shows the difference between the PMFs for
