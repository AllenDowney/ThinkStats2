...
% TODO: add an example using Hist and Pmf, and warn about the
% implicit interpolation when you use Pmf.


\section{Comparing PMFs}
...
first babies and others as bar graphs.  Using the PMF, we can see more
clearly where the distributions differ.  First babies seem to be less
likely than others to arrive on time (week 39) and more likely to be a
late (weeks 41 and 42).
...
difference apparent.  But we can't be sure this effect is real;
...
think that the average class size was higher.  Let's see how
much higher.

First, I'll make a distribution of the actual data, using the
midpoint of each size range:
...
The mean of this distribution is 23.7.  Next we'll compute the biased
distribution as observed by students.
...
The mean of the biased distribution is 29.1, almost 25\% bigger
...
sample of students and ask them the number of students in each of
their classes.
\index{bias!oversampling} \index{oversampling}
...
It's very similar to {\tt BiasPmf}; the only difference is that it
divides each probability by {\tt x} instead of multiplying.

The code for this example is in \url{chap03.py}.
...
distribution function, which is the subject of the next chapter.
