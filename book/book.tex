...
So we can compute the sampling distribution of the observed means
like this:
...
Both samples are drawn from the same distribution, which is the
pool of all live births.  
{\tt SamplingDistMean} takes this sequence of
values and the sample size:
...
It returns the
sampling distribution of the mean of {\tt n} values drawn from
{\tt data}:

{\tt mean} and {\tt var} are the sample mean and variance of
{\tt data}.  We approximate the distribution of the data with
a normal distribution, {\tt dist}.  In this example, remember
that the data are not normally distributed, so this approximation
is not very good.  

But then we compute {\tt dist.Sum(n) / n}, which is the sampling
distribution of the mean of {\tt n} values.  Even if the data are
not normally distributed, the sampling distribution of the mean
...
So we can compute the sampling distribution of the difference:
...
The mean is 0, since we expect two samples from the same distribution
to have the same mean, on average.  The variance of the sampling
distribution is 0.0032.
...
\begin{exercise}
Find the confidence interval for the difference in means.
\end{exercise}
