...
three possible sources of error: measurement error, sampling error,
and modeling error.

In this example, measurement error is probably small.  In general,
people know when they were born, whether they've been married, and
when.  And they can be expected to report this information
accurately.
...
{\tt ResampleSurvival} takes a DataFrame of respondents and {\tt iters},
the number of times to resample.  It computes {\tt ts}, which is the
array of ages where we will evaluate the survival functions.
...
previous sections to estimate the survival curve, and

\item Evaluates the survival curve at each
age in {\tt ts}.  

\end{itemize}

So \verb"ss_seq" is a sequence of evaluated
survival curves.
{\tt PercentileRows} takes this sequence and computes the 5th
and 95th percentiles, returning a 90\% confidence interval for
the survival curve.
...
interval takes into account the sampling weights, but the
estimated survival curve does not.  Apparently the
sampling weights have a substantial effect on the estimate,
which is why the curve does
not lie within the confidence interval.
...
If the relevant characteristics of the repondents are not changing
...
to their decade of birth.  A group like this, defined by date of
birth or similar events, is called a {\bf cohort}, and differences
between such groups are called {\bf cohort effects}.

I use {\tt cmbirth} to identify the decade of birth for each
respondent:
...
Then {\tt } generates survival curves for each decade:
...
Figure~\ref{survival4} shows the result, including data from
the 2002 and 2006--2010 NSFG (see Section\ref{replication}).
Women born in the 50s married earliest, with each successive cohort
marrying later and later.

The shapes of the curves are
similar, with one exception: the 60s cohort was marrying more
slowly than the 50s cohort until they turned 25; between 25 and 30,
they caught up with their predecessors, and the rest of the curves
are nearly identical.  Women born in the 60s turned 25 between 
1985 and 1995.

Remembering that the {\it Newsweek} article I mentioned was published
in 1986, it is tempting to imagine that the article triggered a
marriage boom.  That explanation would be too pat, but it it possible
that the article and the reaction to it were indicative of a mood
that affected the behavior of this cohort.

The estimated survival curves for successive cohorts get shorter
because of missing data.  Women born in the 1970s were 40, at the
oldest, during the most recent NSFG, so their curve ends at age 40.
Similarly, the curve for the 80s cohort ends at age 30, but so far
they are substantially less likely to get married than their
predecessors.  And, based on early returns, the 90s cohort is off to
