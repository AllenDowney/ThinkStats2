...
The result is about 68\%, which means that if the actual difference in
mean pregnancy length is 0.78 weeks, we expect an experiment with this
sample size to yield a negative test 68\% of the time.
...
different subsets of the NSFG data.  You can use {\tt scatter.SampleRows}
...
0.07 and confidence interval (6.70, 6.95).  The estimated slope, in
more compact form, is 0.0175, SE 0.0028, CI (0.0126, 0.0226).
There is almost a factor of two between the low and high ends of
this CI, so it should be considered a rough estimate.

%inter 6.83039697331
%SE, CI 0.0695704914151 (6.7017063350011705, 6.9472713922362352)
%slope 0.0174538514718
%SE, CI 0.00278994757445 (0.012640165265054685, 0.022557267762599829)
...
of the residuals, also known as the {\bf norm of residuals}:

\begin{verbatim}
norm_res = thinkstats2.Std(res)
\end{verbatim}

If you use a linear model to make predictions, the norm of residuals
...
the RMSE of your guess is {\tt Var(ys)}, which is 1.41 lbs.
...
{\tt Var(ys)} if the MSE without it.   So their ratio is the fraction
...
So if $\rho$ is 0.8 or -0.8, $R^2 = 0.64$.
...
power.  In my opinion, norm of residuals is the best representation
of the quality of prediction, especially if it is presented
in relation to {\tt Var(ys)}.
...
only predict only 52\% of variance in IQ.

In terms of norm of residuals, using SAT score to predict IQ
reduces RMSE from 15 points to 10.4 points.  So a correlation of 0.72
yields a reduction in RMSE of only 31\%.

If you see a correlation that looks impressive, remember that $R^2$
(which is generally smaller) is a better indicator of the effect of
the model on MSE, and reduction in RMSE is a better indicator of
predictive power.
...
% TODO: rewrite this using the framework
