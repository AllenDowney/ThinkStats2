...
In some cases it is possible to infer causal relationships using {\bf
  regression analysis}, which is the topic of Chapter~\ref{regression2}.
...
%TODO: show 50% and 90% CIs?

\begin{figure}
% regression.py
\centerline{\includegraphics[height=2.5in]{figs/regression3.pdf}}
\caption{Confidence intervals showing variability in the fitted
line due to sampling error of {\tt inter} and {\tt slope}.}
\label{regression3}
\end{figure}
...
then, it is usually good enough, because p-values don't have to
be precise.
...
sample, but as I mentioned in Section~\ref{nsfg}, it is not.  The NSFG
deliberately oversamples several groups in order to improve the chance
of getting statistically significant results; that is, in order to
improve the power of tests involving these groups.

This survey design is useful for many purposes, but it means that we
cannot use the survey sample to estimate values for the general
...
than mothers of other babies.  \index{birth weight}
\index{weight!birth} \index{National Survey of Family Growth}
\index{NSFG}
