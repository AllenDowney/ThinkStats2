...
In this example the indices are integers from 0 to 13593, but in
...
{\tt Density} takes a value, calls \verb"gaussian_kde.evaluate", and
returns the resulting density.  The word ``Gaussian'' appears in the
name because it uses a filter based on a Gaussian distribution to
smooth the KDE.  \index{density}
...
\[ \goodchi^2 = \sum_i \frac{(O_i - E_i)^2}{E_i} \]
%
One reason the chi-square statistic is a popular choice is that
...
A solution to this exercise is in \verb"chap14ex_soln.py"
