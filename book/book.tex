...
Now we are ready to tackle the {\bf cumulative distribution function}
...
$x$, we compute the fraction of the values in the distribution less
than (or equal to) $x$.
...
should match the original distribution.  Let's check; here's the
code:
...
which evaluates a Gaussian CDF at {\tt x}, with the given parameters.
