...
\newcommand{\thetitle}{Think Stats: Data Science for Programmers}
...
\begin{spacing}{3}
{\huge Think Stats: Data Science for Programmers}\\
{\Large }
\end{spacing}
...
\begin{spacing}{3}
{\huge Think Stats}\\
{\Large Data Science for Programmers}
\end{spacing}
...
{\em Think Stats: Data Science for Programmers} is an
...
means there are packages you need to install.
...
The {\tt Series} class provides a method, \verb"value_counts", that
...
if you look more closely, you notice one value that has to be
...
Using this list as an index into {\tt preg.outcome} selects the
...
{\tt Hist}, that represents histograms and provides the methods
...
about installing {\tt matplotlib}.  \index{{\tt thinkplot}}
\index{matplotlib}

To plot a histogram with {\tt thinkplot}, try this:
...
\end{verbatim}
%
Or we can use the {\tt Pmf} class provided by {\tt thinkstats2}.
...
You can find a solution in \verb"chap03soln.ipynb".
\end{exercise}


For the following exercises, create a file named {\tt chap03ex.py}.
You can find a solution in \verb"chap03soln.py".
...
This code for this chapter is in {\tt cumulative.py}.
For information about downloading and
working with this code, see Section~\ref{code}.
...
{\tt thinkstats2} provides a class named {\tt Cdf} that represents
CDFs.  The fundamental methods {\tt Cdf} provides are:
...
\item[Simulation]: Simulations are often based the distribution
...
{\tt Hist} and {\tt Pmf}, including \verb"__init__", {\tt Values},
...
{\tt Normalize} raises {\tt ValueError}.
...
%So {\tt indices} is one element longer than {\tt bins}.
...
intervals for curves fitted to the high quality price data.
...
pool.  Suppose your friend is and 35 years old and white,
...
\url{http://zmjones.com/static/data/mj-clean.csv} and read it into a
pandas DataFrame:
...
{\tt groupby} is a DataFrame method that returns a GroupBy object;
used in a for loop, {\tt groups} iterates the names of the groups and
the DataFrames that represent them.  Since the values of {\tt quality}
...
My solutions to these exercises are in \verb"chap12soln.py".
...
the fraction of pregnancies that survive until $t$ and then end at
{\tt t}.  To be more precise:
%
\[ \lambda(t) = \frac{S(t) - S(t+1)}{S(t)} \]
%
The numerator is the fraction of lifetimes that end at $t$, which
is also $\PMF(t)$, where $\PMF$ is the distribution of lifetimes.
...
The estimated value of the hazard function at {\tt t} is
...
{\tt evrmarry}: 1 if the respondent had been
...
Then we compute each respondents age when married and age when
interviewed:
...
for the 80s cohort is ends at age 28, and for the 90s cohort
...
``expected'' usually means ``average.''  {\tt SurvivalFunction}
...
Figure~\ref{survival6} (left) shows the expecting remaining pregnancy
...
marriage.  Components with this property are UBNE for "used better
than new in expectation."  That is, the older the part, the longer it
is expected to last.  Newborns and cancer patients are also {\bf UBNE};
their life expectancy increases the longer they live.  Also, people
...
My solutions to these exercises are in \verb"chap13soln.py".
...
of people alive at $t$ who die at $t$.

\item Kaplan-Meier estimation: An algorithm for estimating hazard and
survival functions.

\item cohort: a group of subject defined by an event, like date of
...
resampling, but some of the problems we solved also have
analytic solutions that can be much faster.

I present some of these methods in this chapter, and explain
how they work.  At the end of the chapter, I make some suggestions
...
As an example, let's review the problem from Section~\ref{gorilla}:
...
To answer that question, we'll compute the sampling
distribution of $\xbar$.  In Section~\ref{gorilla} we approximated
the sampling distribution by simulating the experiment (weighing
9 gorillas), computing $\xbar$ for each simulated experiment, and
accumulating the distribution of estimates.

The result is an approximation of the sampling distribution.  We can use
...
Now we'll do the same calculation analytically.  To do that, we can
...
need some notation.  \index{analysis} \index{random variable}
...
Now we have everything we need to compute the sampling distribution
of $\xbar$.  If we assume, temporarily, that $\xbar$ is an accurate
estimate of the population mean, $\mu$, and $S^2$ is an accurate
estimate of the population variance, $\sigma^2$, then the
distribution of gorilla weights is
%
\[ X \sim \normal (\mu, \sigma^2)\]
%
Remember that we compute $\xbar$ by weighing $n$ gorillas, adding up
the total weight, and dividing by $n$.
If we weigh $n$ gorillas, the total weight, $Y$, is distributed
%
\[ Y \sim \normal (n \mu, n \sigma^2) \]
%
using Equation~\ref{eqn1}.  And if we divide by $n$, the sample mean,
$Z$, is distributed
%
\[ Z \sim \normal (\mu, \sigma^2/n) \]
%
using Equation~\ref{eqn2} with $a = 1/n$.
...
And the standard deviation of the sampling distribution, which is the
...
and {\tt sigma}.  For the sampling distribution of $\xbar$,
...
Normal also knows how to multiply and divide by a scalar, using
Equation~\ref{eqn1}:
...
This result is the {\bf Central Limit Theorem} (CLT).  It is one of the
...
(I'll use ``exponential values'' as shorthand for ``values from an
...
\begin{figure}
% normal.py
\centerline{\includegraphics[height=3in]{figs/normal1.pdf}}
...
\begin{figure}
% normal.py
\centerline{\includegraphics[height=3in]{figs/normal2.pdf}}
...
To test that, I'll generate correlated values from an exponential
...
depends on it predecessor: if the previous value is $x$, the mean of
the next value is $x$, with variance {\tt 1-rho**2}.  Note that {\tt
...
As we've seen several times, the apparent difference is about
0.078 weeks:
...
subtraction using Equation~\ref{eqn3}:
...
So can compute the sampling distribution of the difference like this:
...
the total squared deviation from the expected values in a table:
%
\[ \goodchi^2 = \sum_i \frac{(O_i - E_i)^2}{E_i} \]
%
One reason the chi-squared statistic is a popular choice is that
...
\item If replacing a computational method with an analytic method is
  appropriate, use the computational model as a basis of comparison, 
...
A solution to this exercise is in \verb"chap14soln.py"
...
Since the log of a product is the sum of the logs of the
factors,
%
\[ \log w = \log w_0 + \log f_1 + \log f_2 + ... + \log f_n \]
%
By the Central Limit Theorem, the distribution of $\log w$ is
approximately normal, for large $n$, so the distribution of $w$ is
lognormal.
...
random value from the distribution of birthweights, choosing a
...
asked them to rate their contribution to each aspect of the project on
a 7-point scale.
...
We can check for non-linearity by plotting the residuals, as we
did in Section~\ref{residuals}.  First, I extract the exogenous
variable, {\tt years}, and the residuals, and store them in
a DataFrame:
...
Next, I divide the x-axis (years) into bins using {\tt digitize}
and {\tt groupby}, which we saw in Section~\ref{characterizing}:
...
Then for each bin, I compute the average of {\tt exog} and the
distribution of {\tt resid}:
...
Finally, I plot the 25th, 50th, and 75th percentiles of the
residuals:
...
\begin{figure}
% timeseries.py
\centerline{\includegraphics[height=2.5in]{figs/timeseries3.pdf}}
\caption{Residuals of the linear fit.}
\label{timeseries3}
\end{figure}

The result is in Figure~\ref{timeseries3}.  If the long-term trend
is actually linear, the 50th percentile curve should be flat, but it
clearly falls between 0.5 and 1.5 years, and then climbs after that,
which indicates that there is a secondary trend that is not captured
by the model.

This model can be improved by adding a quadratic term, {\tt years**2},
as an explanatory variable, as we did
in Section~\ref{nonlinear}.  You'll have a chance to explore this
option in the exercises at the end of this chapter.  But for now
I will stick with the linear model.

We can use the fitted model to predict prices in the future.
As we saw in Section~\ref{accuracy}, RegressionResults provides {\tt predict},
which takes a DataFrame containing the explanatory variables and
returns a sequence of predictions.  Here's the code:
