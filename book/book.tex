...
weights have a substantial effect on the estimate, so we will have
...
is a sequence of Timestamps, which is converted to a {\tt
  DateTimeIndex}.  Then we get the {\tt year} and compute the
decade.

To take into account the sampling weights, and also to show
variability due to sampling error, I resample the data 11 times,
group respondents by decade of birth, and plot survival curves:
...
so I resample each of them separately and then use {\tt concat}
...
The next step is to group the respondents by decade; then {\tt
  EstimateSurvivalByDecade} generates survival curves for each decade:
...
\item Women born in the 50s married earliest, with each successive
  cohort marrying later and later, at least until age 25 or so.
...
the 60s cohort, and at age 44 they are substantially more likely to
have married.
...
That explanation would be too pat, but it it possible that the article
...
\item Women both in the 80s are even less likely to marry before
age 25.  What happens after that is not clear; for more data, we
have to wait for the next cycle.

\end{itemize}

But in the meantime we can make some predictions.
...
previous cohort.  
HazardFunction provides a method, {\tt Extend}, that
copies the tail from a another longer HazardFunction:
...
As we saw in Section~\ref{???}, the HazardFunction contains a Series
that maps from $t$ to $\lambda(t)$.  {\tt Extend} finds {\tt last},
which is the last index in {\tt self}, selects the values from
{\tt other} that come later than {\tt last}, and appends them
onto {\tt self.series}.

Now we can extend the HazardFunction for each cohort with values
...
Figure~\ref{survival5} shows the results; I've excluded the 50s cohort
